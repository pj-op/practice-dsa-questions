...
public class TestSrt {
    public static void main(String[] args) {
        ...
        mrsrt(arr);
        System.out.println(Arrays.toString(arr));
        System.out.println(isSrted(arr, arr.length));

        Set<List<Integer>> ans = new HashSet<>();
        ans.stream().collect(Collectors.toList());*/
        /*
        recTest(10);
        System.out.println(digitRoot(5569399));
         */
        /*//System.out.println(areIsomorphic("xyz", "bba"));
        String s = "1abc23";
        System.out.println(findSum(s));*/
//        System.out.println(concatenatedString("aaaa", "bbbb"));
        /*
        ...
        System.out.println(printIntersection(arr1, arr2, arr1.length, arr2.length));
         */
        /*int[] arr = {2,1,2};
...
//        System.out.println(Arrays.deepToString(booleanMatrix(matrix)));*/
...
//        After that replace N with N + 5 until N regains its initial value.
        ...
        System.out.println(pattern(n, n, ans));*/
        /* //Max SumSubArray
        int[] a = {8, -8, 9, -9, 10, -11, 12, 8, -8, 9, -9, 10, -11, 12};
...
//                {1, -2, 0, -1, -2, 1, -4, 2, -3};
    ...
    private static int missingNumber(int[] arr, int size) {
        ...
        for (int i = 0; i < size; i++) {
            if (arr[i] > 0) {
                if ((i + 1) < size && arr[i] == arr[i + 1]) continue;
                if (arr[i] != num) {
                    return num;
                }
                num++;
    ...
    private static int circularSubarraySum(int[] a) {
        // Your code here
        int sum = 0;
        int n = a.length;
        int startI = -1;
        ...
        int[] ans = new int[endI - startI];
        AtomicInteger idx = new AtomicInteger(0);
        IntStream.range(startI, endI).forEach(i -> ans[idx.getAndIncrement()] = a[i]);
    ...
    private static List<Integer> pattern(int original, int n, List<Integer> ans) {
        while (n > 0) {
            ans.add(n);
            n -= 5;
        }
    ...
    private static int[][] booleanMatrix(int[][] matrix) {
        // code here
        int n = matrix.length;
    ...
    private static int lis(int[] arr) {
        ...
        for (int i = 0; i < n; i++) {
            lis[i] = 1; //As by default first element is always an subsequence
            for (int j = 0; j < i; j++) {
                if (arr[j] < arr[i]) {
    ...
    private static void findKthLargest(int[] arr, int k) {
        Queue<Integer> pq = new PriorityQueue<>(Comparator.comparing(Integer::intValue).reversed());
        for (int ele : arr) {
            if (!pq.contains(ele))
                pq.add(ele);
        }
    ...
    private static ArrayList<Integer> printIntersection(int[] arr1, int[] arr2, int n, int m) {
        // add your code here
        int i = 0, j = 0;
    ...
    public static boolean areIsomorphic(String str1, String str2) {
        ...
        for (int i = 0; i < n1; i++) {
            ...
            if (!map1.containsKey(c1) && !map2.containsKey(c2)) {
                map1.put(c1, c2);
                map2.put(c2, c1);
            } else if ((map1.containsKey(c1) && c2 != map1.get(c1)) || (map2.containsKey(c2) && c1 != map2.get(c2))) {
                return false;
            }
    ...
    private static int digitRoot(int n) {
        if (n == 0) return 0;
        int ans = (n % 10) + digitRoot(n / 10);
        return (ans > 9) ? digitRoot(ans) : ans;
    }

    private static void recTest(int num) {
        if (num <= 0) {
    ...
    private static int isSrted(int[] arr, int n) {
        if (n == 1) return 1;
        return (arr[n - 1] < arr[n - 2]) ? 0 : isSrted(arr, n - 1);
    }

    private static void mrsrt(int[] arr) {
        ...
        IntStream.range(mid, n).forEach(i -> rightArr[i - mid] = arr[i]);

        mrsrt(leftArr);
        mrsrt(rightArr);

        mrChunks(leftArr, rightArr, arr);
    }

    private static void mrChunks(int[] leftArr, int[] rightArr, int[] arr) {
        int i = 0, j = 0, k = 0;
